...
def build_dashboard(visual_symbol, symbol, position, current_price, orders, balance, price_changes, all_positions=None):
    ...
    # Balance panel - calculate total unrealized PnL from all positions
    ...
    total_unrealized_pnl = unrealized_pnl  # Current position PnL
    if all_positions and len(all_positions) > 1:
        for pos in all_positions:
            if pos['symbol'] != symbol:  # Don't double-count current position
        ...
        while True:
            try:
                ...
                volume_panel, info_panel, balance_panel, timer_panel, header, progress_renderables = build_dashboard(
                    visual_symbol, symbol, position, current_price, orders, balance, price_changes, positions
                )
