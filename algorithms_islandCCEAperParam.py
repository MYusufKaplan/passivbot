    ...
    with open(WATCH_PATH, "a") as f, contextlib.redirect_stdout(f), contextlib.redirect_stderr(f):
        ...
        for island in islands:
            ...
            for specialist in island['specialists']:
                # Create full individual by combining specialist with global vector
                full_solution = global_best_vector.copy()
                full_solution[param_idx] = specialist
        ...
        with Progress(
            ...
            for island_id, island in enumerate(islands):
                # Abbreviate parameter nor display
                param_name = island['param_name']
                if param_name.startswith('long_'):
                    param_name = param_name[5:]  # Remove 'long_' prefix
                
                task = progress.add_task(
                    f"🏝️ {param_name} | Best: {island_best_fitnesses[island_id]:.6e}", 
            ...
            for fitness in pool.imap_unordered(evaluate_solution, all_args):
                ...
                for island_id, (start, end) in enumerate(island_ranges):
                    if start <= result_idx < end:
                        ...
                        param_name = islands[island_id]['param_name']
                        if param_name.startswith('long_'):
                            param_name = param_name[5:]  # Remove 'long_' prefix
                        ...
                        progress.update(
                            ...
                            advance=1,
                            description=f"🏝️ {param_name} | Best: {island_best_fitnesses[island_id]:.6e}"
                        )
                        break
        ...
        for island_id, island in enumerate(islands):
            ...
            if current_gen_island_best_fitness < island['best_fitness']:
                ...
                if island['best_specialist'] not in island['specialists']:
                    log_message(f"⚠️ ERROR: All-time best specialist {island['best_specialist']} not found in current specialists for {island['param_name']}", emoji="⚠️")
            ...
            # Store fitness mapping for selection
            island['fitness_map'] = dict(zip(island['specialists'], island_fitnesses))
...
def eaMuPlusLambda(population, toolbox, mu, lambda_, cxpb, mutpb, ngen, evaluator,
    ...
    if os.path.exists(checkpoint_path):
        ...
        for island in islands:
            ...
            if param_name in integer_params:
                island['is_integer'] = True
                # Ensure current best_specialist is integer
                island['best_specialist'] = round(island['best_specialist'])
        ...
        # Create islands - one per optimizable parameter
        for param_name in optimizable_bounds.keys():
            param_idx = all_param_names.index(param_name)
            min_val, max_val = optimizable_bounds[param_name]
            
            # Create specialists (single float/int values) for this parameter
            # Use initial value from optimize.json if available, otherwise use middle value
            if initial_values and param_name in initial_values:
                initial_val = initial_values[param_name]
                if param_name in integer_params:
                    initial_val = round(initial_val)
                    # Generate random specialists and include the initial value
                    specialists = [random.randint(int(min_val), int(max_val)) for _ in range(island_size - 1)]
                    specialists.append(initial_val)  # Include initial value as a specialist
                    best_specialist = initial_val
                else:
                    # Generate random specialists and include the initial value
                    specialists = [random.uniform(min_val, max_val) for _ in range(island_size - 1)]
                    specialists.append(initial_val)  # Include initial value as a specialist
                    best_specialist = initial_val
            else:
                # Fallback to middle value
                if param_name in integer_params:
                    middle_val = round((min_val + max_val) / 2)
                    specialists = [random.randint(int(min_val), int(max_val)) for _ in range(island_size - 1)]
                    specialists.append(middle_val)  # Include middle value as a specialist
                    best_specialist = middle_val
                else:
                    middle_val = (min_val + max_val) / 2
                    specialists = [random.uniform(min_val, max_val) for _ in range(island_size - 1)]
                    specialists.append(middle_val)  # Include middle value as a specialist
            ...
            log_message(f"  🏝️ {param_name}: {island_size} specialists [{min_val:.4f}, {max_val:.4f}]", emoji="🏝️")
        ...
        # Set each island's best specialist to the corresponding global vector value
        for island in islands:
            param_idx = island['param_idx']
            island['best_specialist'] = global_best_vector[param_idx]
    ...
    for gen in range(start_gen, ngen + 1):
        ...
        for island in islands:
            ...
            is_integer = island.get('is_integer', False)
            
            # Generate mu/island_count mutated specialists
            new_specialists = []
            
            # First, preserve the best specialist from last generation (elitism)
            new_specialists.append(best_specialist)
            
            # Debug: Ensure the best specialist is actually in the list
            if best_specialist not in new_specialists:
                log_message(f"⚠️ ERROR: Best specialist {best_specialist} not in new_specialists for {island['param_name']}", emoji="⚠️")
            
            # Then generate (island_size - 1) mutated specialists
            for _ in range(island_size - 1):
                mutated = mutate_specialist(best_specialist, bounds, mutation_strength=mutpb, is_integer=is_integer)
                new_specialists.append(mutated)
        ...
        for island in islands:
            ...
            if test_global_fitness < global_best_fitness:
                ...
                island['stagnation'] = 0  # Reset island stagnation
                proposed_global_vector[param_idx] = current_gen_best_specialist
                any_improvement = True
                log_message(f"✅ {island['param_name']}: {current_gen_best_specialist:.6f} improves global fitness to {test_global_fitness:.6e}", emoji="✅")
            else:
                # No improvement for this island
                island['stagnation'] += 1  # Increment island stagnation
                log_message(f"❌ {island['param_name']}: No improvement (stagnation: {island['stagnation']})", emoji="❌")
        ...
        if verbose:
            ...
            for i, (param_name, value) in enumerate(zip(all_param_names, global_best_vector)):
                ...
                island_stagnation = 0
                for island in islands:
                    if island['param_name'] == param_name:
                        island_stagnation = island['stagnation']
                        break
                
                if param_name in integer_params:
                    global_vector_info += f"\n                  📊 {param_name}: {int(value)} (stag: {island_stagnation})"
                else:
                    global_vector_info += f"\n                  📊 {param_name}: {value:.6f} (stag: {island_stagnation})"
