...
def build_dashboard(visual_symbol, symbol, position, current_price, orders, balance, price_changes):
    ...
    info.add_row(f"[bold]✅ Realized PnL:[/] {realized_pnl:.5f}")
    ...
    balance_try = (balance_usdt + unrealized_pnl + realized_pnl) * fetch_usdt_try_price()
    profit_try = balance_try - 209000
    balance_table.add_row(f"[bold]💰 Balance TRY:[/] {balance_try:.2f} (Profit: {profit_try:+.2f})")
