        ...
        for (&coin_idx, &timestamp_opt) in &self.position_timestamps {
            if let Some(timestamp) = timestamp_opt {
                ...
                if hours_since_trade >= max_hours_stale {
                    println!("\x1b[35m🕰️  BANKRUPTCY (INACTIVITY - STALE POSITION): Stale position on coin {} ({}) at timestep {}/{} for {} hours (max: {})\x1b[0m", 
...
fn analyze_backtest_basic(fills: &[Fill], equities: &Vec<f64>, bankruptcy_timestamp: Option<usize>) -> Analysis {
    ...
    if fills.len() <= 1 {
        if bankruptcy_timestamp.is_none() {
            println!("\x1b[36m🤔 NO-TRADE STRATEGY WITHOUT BANKRUPTCY: fills={}, this should not happen with inactivity detection!\x1b[0m", fills.len());
        }
