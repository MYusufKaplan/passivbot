...
def log_message(message, emoji=None):
    """Utility function to print logs with colors and emojis using rich"""
...
def eaMuPlusLambda(population, toolbox, mu, lambda_, cxpb, mutpb, ngen,
    ...
    total_time = 0  # Track total time for all generations
    # Begin the generational process
    for gen in range(start_gen, ngen + 1):
        gen_start_time = time.time()  # Track the start time of the generation
        ...
        # Update the hall of fame with the generated individuals
        if halloffame is not None:
            old_best = halloffame[0] if halloffame else None
            halloffame.update(offspring)
            new_best = halloffame[0] if halloffame else None

            # Print message if a new best is found
            if new_best != old_best:
                log_message(f"New best individual found in generation {gen}: {new_best.fitness.values[0]} {new_best.fitness.values[1]}", emoji="🏅")
        ...
        # Print average time per iteration (in seconds, minutes, hours)
        avg_time_per_iter = total_time / (gen - start_gen + 1)
        avg_seconds = avg_time_per_iter
        avg_minutes = avg_time_per_iter / 60
        avg_hours = avg_time_per_iter / 3600

        log_message(
            f"Average time per iteration: {avg_seconds:.2f} seconds "
            f"({avg_minutes:.2f} minutes, {avg_hours:.2f} hours)",
            emoji="⏳"
        )

    # Track total time after all generations
    end_time = time.time()
    ...
    log_message(
        f"Total time for optimization: {total_seconds:.2f} seconds "
        f"({total_minutes:.2f} minutes, {total_hours:.2f} hours)",
        emoji="🕒"
    )
